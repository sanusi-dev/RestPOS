...
2. `make ruff-lint` and `make npm-type-check` (templates touched? only Python changed — lint
   suffices).
3. Unit tests for the specific scenarios:
   - Cancel a purchase receipt after a higher-priced receipt blended the bin (drift leg
     posted to variance account).
...
4. `make migrations` check: no pending schema changes (the settings FK migration was
   regenerated per H2).
5. Update the status column for each item to `✅` in this file.
...
| H1 | HIGH | ⬜ Dropped per owner — no docstring litter; migration is intentionally `RunPython(wipe, noop)` and only safe on pre-prod DB (sign-off required separately). |
...
| M1 | MEDIUM | ⬜ Dropped — per follow-up review: handful of Bin rows, property is clearer than `F()` annotation; no action. |
