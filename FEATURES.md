...
| 61 | Supplier payables | **Implemented (Phase 2 / §4.1 + PWAC rework).** Supplier master, supplier invoices (stock and expense lines), supplier payments fully allocated to outstanding invoices, and accounts-payable balances per supplier. Stock invoices must link to a purchase receipt and post Dr GRNI / Cr payable at the same rate (rate equality enforced); expense lines post Dr expense / Cr payable even when linked. Purchase receipts post Dr SIH / Cr GRNI (accrual); market purchases via Stock Entry post Dr SIH / Cr expense directly (no GRNI). Payments post Dr payable / Cr cash-bank; cancellation reverses. |
